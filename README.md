...
> 所有结论由大模型生成，**不构成投资建议**。控制台默认只监听 `127.0.0.1`；局域网模式强制密码登录（口令取自 `--password`、环境变量 `AIPLAN_WEBUI_PASSWORD` 或本地 `config/webui配置.json`，源码里不存默认口令），并且防火墙只放行本地子网，仍**不要**暴露到公网。
...
需要让手机或同局域网设备访问时，双击 **`启动WebUI局域网.cmd`**：首次会请求一次管理员权限添加仅限本地子网的防火墙规则，
随后在启动窗口打印本机/局域网地址与本次生效的访问口令。手机建议直接打开打印出的 `http://电脑IP:8765/m` 手机专用页，输入启动窗口里那串密码即可登录；
想固定口令，就写进 `config/webui配置.json`（`{"密码": "..."}`，该文件已忽略、不会入库）。
桌面页右上角也提供「手机版」入口。桌面与手机版默认进入总控台。
...
WebUI 同步按钮下方有可折叠的实时日志：运行中自动展开并逐行打印，完成、失败或中断后自动收起。
脚本不登录、不保存密码、不调用交易接口；非交易日只取资金和持仓，直接跳过“当日成交”页以避免无意义的复制验证。若交易日仍出现复制验证码：WebUI 会弹出验证码图片，人工填写后原任务继续；命令行运行则在终端提示输入。超时、校验失败或客户端未启动时不会覆盖任何业务文件。
...
`node tests/ui/ui_smoke.mjs` —— 验证桌面默认总控台，点开 9 个页面并断言各自渲染出真实内容、检查报告页三个页签与
K 线画布、荐股筛选联动；`node tests/ui/mobile_smoke.mjs` 会以三种手机视口检查 `/m` 九页、底部导航、
横向溢出与写入确认。两者都截图到 `build/` 并要求 0 个 console error。
