...
（按买入金额 × 止损距离折算，并给出应下调到的金额）；4. 买入/加仓触发价 ≥ 止损价；
5. 卖出股数 ≤ 可用余额（**T+1：冻结/当日买入不可卖**）；6. 股数为 100 的整数倍；
7. 现金比例 ≥ 下限（并入其它持仓市值）；8. 费用口径（佣金双边含最低 5 元、印花税仅卖出 A 股、
过户费仅 A 股，**ETF 免印花税与过户费**）。

实测有效：曾抓出模型计划「减仓 100 股」但账户 `可用余额=0`（当日买入冻结）的 T+1 违规。
...
`最大加仓次数`、`佣金费率_pct`、`佣金最低_元`、`印花税率_pct`、`过户费率_pct`、`风险偏好`、`api_key`（可空）。
