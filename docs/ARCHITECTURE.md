...
webserver  ← __main__（python -m webui）
   ...
   ├── mktdata    大盘评分取数层：中证指数官网（沪深300 / 中证500 的 PE 十年序列与分位）、
   │               东财（沪深300 日K/月K、上证综指+深证综指日K、指数资金流历史、全市场汇总、
   │               M2 与 3 个月拆借报表、美元指数/标普500）、pan（中债10Y、两融、广度）、
   │               人民币汇率；「近5/10/20 日」类能回抓就回抓、抓不到逐日积累（data/cache/mkt/hist/）
   ├── mktscore   大盘评分（纯函数）：照《大盘评分逻辑.txt》的 6 模块 27 分项阈值打分，
   │               机械分 = 实得 ÷ 可得 × 100，总分 = 机械分×0.7 + 模型分×0.3；
   │               另含模型盲评用的原始证据渲染（不含机械分）与事实包片段 fact_block()
...
大盘快照页读最新荐股产物给四档打法各推一只（与荐股页同一个 `run_pick` 与同一组默认参数，
页面写明这条口径）。大盘评分另走两条任务：`jobs.start_market_job("mkt_score")` 机械重算
（`mktdata.collect` → `mktscore.score`，全量模式含宽度样本与全市场汇总，零成本、可中断）、
`jobs.start_market_job("mkt_read")` 模型盲评（只给原始数据、不给机械分与阈值，出 0-100 分 +
精简解读，落 `data/ai/market/read_<日期>/`）；`/api/market` 只读当天缓存，从不在页面里调模型。
