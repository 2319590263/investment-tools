...
webserver  ← __main__（python -m webui）
   ...
   │     └── pickrank  荐股榜：最新产物的推荐度排序 + 两级筛选树（纯函数）
...
4. **总控台轮询**：前端按所选档位（10s ~ 10min）请求 `/api/overview?refresh=1`——每档 1 次东财批量报价，分时每标的最多 60 秒一次（分钟线每分钟才变）、同轮串行间隔 300ms；非交易时段不发请求，连续失败按 2 倍退避（上限 5 分钟）。同一份响应里 `pickrank` 把**最新一份**荐股产物组装成榜单（推荐度 = 机械分 + 首推加成 8/6/4、上限 100），榜单代码并进**同一次**批量报价（只报价、不取分时，失败按产物快照价并标注）。
