...
def plan_items(payload):
    ...
    给页面用两件事：① **精确价**（区间按统一口径收敛成一个可执行价位，用户批注 5）；
    ② 过滤掉没信息量的「失效条件」（只重复触发价反面的那种，用户批注 6）。
    """
    ...
    for i, raw in enumerate(plan.get("计划") or []):
        ...
        out.append({"编号": i + 1, "优先级": raw.get("优先级"), "动作": raw.get("动作"),
                    "触发条件": raw.get("触发条件"), "价格区间": rng,
                    "精确价": trigger_price(rng, act),
...
TRACK_PROMPT = (
    ...
    "3. 每条都要有可核对的「触发条件」（价格 / 量比 / 均线 / 板块资金等）。「失效条件」必须是"
    "**触发条件之外**的新信息（例如「跌破 MA20 且量能萎缩」「板块资金转为净流出」「公告出现立案」）；"
    "禁止写「价格 < 触发价」这种与触发条件互为反面的废话——没有真正不同的失效信号就写 null，"
    "页面会显示「—」而不是凑字数。\n"
